...
|===
|Placeholder |Description |Example |Files
|`+cloudguard-server+` |Human-readable project name |`+My Project+`
...
|`+CLOUDGUARD_SERVER+` |Uppercase identifier (for Idris2 modules, C
macros) |`+MY_PROJECT+` |ABI-FFI-README.md, src/abi/_.idr, ffi/zig/_.zig

|`+cloudguard_server+` |Lowercase identifier (for C symbols, filenames)
|`+my_project+` |ABI-FFI-README.md, ffi/zig/*.zig

|`+cloudguard-server+` |Repository name (slug) |`+my-project+`
