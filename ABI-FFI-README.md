...
# CLOUDGUARD_SERVER ABI/FFI Documentation
...
│  generated/abi/cloudguard_server.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcloudguard_server.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── cloudguard_server.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cloudguard_server.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cloudguard_server.h
```
...
```c
#include "cloudguard_server.h"

int main() {
    void* handle = cloudguard_server_init();
    if (!handle) return 1;

    int result = cloudguard_server_process(handle, 42);
    if (result != 0) {
        const char* err = cloudguard_server_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcloudguard_server -L./zig-out/lib
```

### From Idris2

```idris
import CLOUDGUARD_SERVER.ABI.Foreign
...
```rust
#[link(name = "cloudguard_server")]
extern "C" {
    fn cloudguard_server_init() -> *mut std::ffi::c_void;
    fn cloudguard_server_free(handle: *mut std::ffi::c_void);
    fn cloudguard_server_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cloudguard_server_init();
        assert!(!handle.is_null());

        let result = cloudguard_server_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcloudguard_server = "libcloudguard_server"

function init()
    handle = ccall((:cloudguard_server_init, libcloudguard_server), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cloudguard_server_process, libcloudguard_server), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cloudguard_server_free, libcloudguard_server), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cloudguard_server.h
   ```
